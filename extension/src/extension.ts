...
async function loadPersistedForDocument(doc: vscode.TextDocument): Promise<void> {
  if (!getPersistResultsEnabled()) return;
  if (!isSupportedDocument(doc)) return;
  const key = doc.uri.toString();
  if (findingStore.has(key)) return;
...
async function loadPersistedForOpenDocuments(): Promise<void> {
  for (const doc of vscode.workspace.textDocuments) {
    await loadPersistedForDocument(doc);
  }
}
...
async function restorePersistedResults(options?: { showInfo?: boolean }): Promise<void> {
  ...
  updateSummaryStatusBar();
  findingsProvider?.refresh();
  void loadPersistedForOpenDocuments();
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor((editor) => {
      if (editor) applyCategoryDecorationsForEditor(editor);
    })
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument((doc) => {
      void loadPersistedForDocument(doc);
    })
  );
